...
function sanitizeUser(user) {
  ...
  const role = isSistemasAdmin ? "ADMIN" : user.role;
...
function saveToDisk() {
  try {
    ...
    console.warn("[DIMER DB] Could not write to disk cache (normal in serverless/readonly mode):", err?.message);
  }
}
...
function getAuthenticatedUser(req) {
  ...
  const headerId = req.headers["x-user-id"]?.trim();
...
function createApp() {
  ...
  app2.get(["/api/diagnostic", "/diagnostic"], (req, res) => {
    res.json({
      status: "operational",
  ...
  app2.post(["/api/auth/login", "/auth/login", "/api/login", "/login"], (req, res) => {
    try {
      const body = req.body || {};
      ...
      if (!email || !password) {
        ...
        return res.status(400).json({ success: false, error: "Correo electr\xF3nico y contrase\xF1a requeridos" });
      }
      ...
      if (!userRecord) {
        ...
        return res.status(401).json({ success: false, error: "No existe una cuenta registrada con este correo. Por favor reg\xEDstrate." });
      }
      if (userRecord.status === "INACTIVO") {
        console.warn(`[AUTH LOGIN] Inactive account rejected: ${userRecord.id}`);
        return res.status(403).json({ success: false, error: "Esta cuenta est\xE1 inactiva. Contacta al Administrador." });
      }
      ...
      if (userRecord.passwordHash && userRecord.salt) {
        ...
        if (!isValid) {
          ...
          return res.status(401).json({ success: false, error: "Contrase\xF1a incorrecta. Verifica tus datos e intenta de nuevo." });
        }
      }
      ...
      res.status(500).json({ success: false, error: err.message || "Error en el inicio de sesi\xF3n" });
    }
  });
  ...
  app2.post("/api/switch-user", (req, res) => {
  ...
  app2.post("/api/departments", (req, res) => {
    try {
      const currentUser = getCurrentUser();
  ...
  app2.put("/api/departments/:id", (req, res) => {
    try {
      const currentUser = getCurrentUser();
      if (!hasPermission(currentUser, "administrar_departamentos") && currentUser.role !== "ADMIN") {
        return res.status(403).json({ error: "No tienes permiso para administrar departamentos" });
  ...
  app2.get("/api/bosses", (req, res) => {
    res.json(BOSSES);
  });
  app2.post("/api/bosses", (req, res) => {
    try {
      const currentUser = getCurrentUser();
  ...
  app2.put("/api/bosses/:id", updateBossHandler);
  app2.patch("/api/bosses/:id", updateBossHandler);
  app2.post("/api/bosses/:id", updateBossHandler);
  app2.delete("/api/bosses/:id", (req, res) => {
    try {
      const currentUser = getCurrentUser();
      if (!hasPermission(currentUser, "administrar_jefes") && currentUser.role !== "ADMIN") {
  ...
  app2.get("/api/permissions", (req, res) => {
  ...
  app2.get("/api/approval-tokens/:token", (req, res) => {
  ...
  app2.post("/api/approval-tokens/:token/action", async (req, res) => {
    try {
      const { token } = req.params;
  ...
  app2.get("/api/requests/:id", (req, res) => {
    const { id } = req.params;
    ...
    res.json({
      ...
      canApprove: currentUser ? currentUser.email.toLowerCase() === request.bossEmail.toLowerCase() || currentUser.role === "ADMIN" || currentUser.email.toLowerCase() === "sistemas@dimer.com.mx" || hasPermission(currentUser, "aprobar_solicitudes") : false
    });
  });
  app2.put("/api/requests/:id", (req, res) => {
    try {
      const { id } = req.params;
      const currentUser = getAuthenticatedUser(req) || getCurrentUser();
      const request = TRAVEL_REQUESTS.find((r) => r.id === id || r.folio === id);
      ...
      if (comments !== void 0) request.comments = comments ? comments.trim() : null;
      if (bossEmail) {
        request.bossEmail = bossEmail.trim().toLowerCase();
        request.bossName = bossName || request.bossEmail;
        if (bossId) request.bossId = bossId;
      }
      if (transportCost !== void 0) request.transportCost = Number(transportCost) || 0;
      if (hotelCost !== void 0) request.hotelCost = Number(hotelCost) || 0;
      if (foodCost !== void 0) request.foodCost = Number(foodCost) || 0;
      if (miscCost !== void 0) request.miscCost = Number(miscCost) || 0;
      if (amountRequested !== void 0 && !isNaN(Number(amountRequested))) {
        request.amountRequested = Number(amountRequested);
      } else if (transportCost !== void 0 || hotelCost !== void 0 || foodCost !== void 0 || miscCost !== void 0) {
        request.amountRequested = (request.transportCost || 0) + (request.hotelCost || 0) + (request.foodCost || 0) + (request.miscCost || 0);
      }
      if (isAdmin && amountAuthorized !== void 0) {
  ...
  app2.patch("/api/requests/:id", (req, res, next) => {
    const fn = app2._router.stack.find((layer) => layer.route && layer.route.path === "/api/requests/:id" && layer.route.methods.put);
    if (fn) {
      return fn.handle(req, res, next);
    }
    res.status(404).json({ error: "Endpoint no disponible" });
  });
  app2.delete("/api/requests/:id", (req, res) => {
    try {
      const { id } = req.params;
      const currentUser = getAuthenticatedUser(req) || getCurrentUser();
  ...
  app2.get("/api/approval/token-action", async (req, res) => {
    try {
  ...
  app2.get(["/api/approval-response/:token/:decision", "/approval-response/:token/:decision"], (req, res) => {
    ...
    const fn = app2._router.stack.find((layer) => layer.route && layer.route.path === "/api/approval/token-action" && layer.route.methods.get);
    if (fn) {
      return fn.handle(req, res);
    }
    res.redirect(`/api/approval/token-action?token=${token}&action=${action}`);
  });
  app2.get("/approval-response", (req, res) => {
  ...
  app2.post("/api/requests/:id/cancel", (req, res) => {
    try {
      const { id } = req.params;
      const currentUser = getAuthenticatedUser(req) || getCurrentUser();
      const request = TRAVEL_REQUESTS.find((r) => r.id === id || r.folio === id);
  ...
  app2.post("/api/requests/:id/approve", async (req, res) => {
    try {
      const { id } = req.params;
  ...
  app2.post("/api/requests/:id/reject", async (req, res) => {
    try {
      const { id } = req.params;
  ...
  app2.post("/api/requests/:id/request-correction", async (req, res) => {
    try {
      const { id } = req.params;
      const { comments } = req.body;
      const currentUser = getCurrentUser();
  ...
  app2.post("/api/requests/:id/pay", (req, res) => {
    const { id } = req.params;
  ...
  app2.post("/api/requests/:id/finalize", (req, res) => {
    const { id } = req.params;
  ...
  app2.get("/api/audit-logs", (req, res) => {
  ...
  app2.get("/api/outbox", (req, res) => {
    res.json(outboxLogs);
  });
  app2.get("/api/smtp/status", (req, res) => {
    const host = process.env.SMTP_HOST?.trim() || process.env.EMAIL_HOST?.trim() || "smtp.gmail.com";
  ...
  app2.post("/api/smtp/test", async (req, res) => {
  ...
  app2.get("/api/code-artifacts", (req, res) => {
  ...
  app2.use((err, req, res, next) => {
    ...
    res.status(500).json({
      ...
      detail: err?.message || "Error no controlado en la ejecuci\xF3n del servidor."
    });
  });
  return app2;
}
