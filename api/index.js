...
function buildVerificationEmailHtml(p) {
  return layout("C\xF3digo de verificaci\xF3n Dimer", `<p>Hola <strong>${esc(p.name)}</strong>.</p><p>Tu c\xF3digo para verificar ${esc(p.email)} es:</p><p style="font-size:36px;text-align:center;letter-spacing:8px"><strong>${esc(p.code)}</strong></p><p>V\xE1lido por ${p.expiresMinutes || 15} minutos.</p>`);
}
function buildNewAccountAdminEmailHtml(p) {
  return layout("Nueva cuenta registrada", `<p><strong>${esc(p.user.name)}</strong> registr\xF3 ${esc(p.user.email)}.</p><p>Departamento: ${esc(p.user.department)}<br>Rol inicial: ${esc(p.user.role)}<br>Fecha: ${esc(p.registeredAt)}</p>`);
}
function buildTokenApprovalResultPageHtml(p) {
  const r = p.request;
  return layout(p.status === "APROBADA" ? "Solicitud aprobada" : p.status === "RECHAZADA" ? "Solicitud rechazada" : "Resultado del dictamen", `<p>${esc(p.errorMessage || `Estado: ${p.status}`)}</p>${r ? `<p><strong>Folio:</strong> ${esc(r.folio)}<br><strong>Solicitante:</strong> ${esc(r.requesterName)}<br><strong>Procesado por:</strong> ${esc(p.processedBy || r.approvedBy || r.rejectedBy || r.bossEmail)}<br><strong>Fecha:</strong> ${esc(p.processedAt || (/* @__PURE__ */ new Date()).toISOString())}</p>` : ""}`);
}
