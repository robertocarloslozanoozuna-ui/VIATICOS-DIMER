...
import { getBossById, generateNextFolio, createApprovalToken, insertRequest, recordAuditLog, getUserById, sanitizeUser } from './db';
import { buildBossApprovalEmailHtml, sendEmail } from './mailService';
import type { TravelRequest, User } from '../src/types';
...
async function createRequest(req: Request, res: Response) {
  ...
  try {
    ...
    try { mailResult = await sendEmail({ to: bossEmail, subject: `SOLICITUD POR AUTORIZAR - Folio ${folio}`, html, requestId: id, folio }); }
    catch (e) { mailResult = { success: false, logId: `MAIL-${Date.now()}`, status: 'FALLIDO', error: e instanceof Error ? e.message : String(e) }; console.error('[DIMER REQUEST EMAIL ERROR]', e); }
