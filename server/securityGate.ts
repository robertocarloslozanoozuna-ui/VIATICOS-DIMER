...
async function handleApproval(req:Request,res:Response){
  ...
  if(isConfirm && req.method==='POST'){
    const body=req.body&&typeof req.body==='object'?req.body:{};
    token=String(body.token||token); action=String(body.action||action); url.searchParams.set('reason',String(body.reason||''));
  }
  ...
  try{
    const result=await processApprovalTokenAction(token,decision,undefined,reason||null);
    ...
    if(decision==='APROBADA'&&user){
      ...
      await sendEmail({to:user.email,subject:`SOLICITUD DE VIÁTICOS APROBADA - ${r.folio}`,html,requestId:r.id,folio:r.folio});
      await sendEmail({to:'sistemas@dimer.com.mx',subject:`SOLICITUD DE VIÁTICOS APROBADA - ${r.folio}`,html,requestId:r.id,folio:r.folio});
      const fin=process.env.FINANZAS_EMAIL||'finanzas@dimer.com.mx';
      if(fin.toLowerCase()!=='sistemas@dimer.com.mx')await sendEmail({to:fin,subject:`SOLICITUD DE VIÁTICOS APROBADA - ${r.folio}`,html,requestId:r.id,folio:r.folio});
    }else if(decision==='RECHAZADA'&&user){
      await sendEmail({to:user.email,subject:`SOLICITUD DE VIÁTICOS RECHAZADA - ${r.folio}`,html:`<p>Su solicitud <strong>${escapeHtml(r.folio)}</strong> fue rechazada.</p><p>${escapeHtml(r.comments||reason)}</p>`,requestId:r.id,folio:r.folio});
    }
...
export async function securityGate(req:Request,res:Response,next:(err?:unknown)=>void){
  const path=req.path;
  if(path==='/api/approval/token-action'||path==='/api/approval/token-action/confirm'||path.startsWith('/approval-response/')||path.startsWith('/api/approval-response/')){
  ...
  if(path.startsWith('/api/')){
    ...
    if(ADMIN_EXACT.has(path)&&user.role!=='ADMIN')return res.status(403).json({error:'Permiso de administración requerido'});
    ...
    if(path==='/api/roles'&&user.role!=='ADMIN')return res.status(403).json({error:'Permiso de administración requerido'});
