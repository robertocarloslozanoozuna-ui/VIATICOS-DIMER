...
export default function App() {
  ...
  const getTabTitle = () => {
    ...
    switch (activeTab) {
      ...
      case 'mis-solicitudes':
        return isSoloLectura ? 'Consulta Exclusiva de Solicitudes Aprobadas' : userRole === 'SOLICITANTE' ? 'Mis Solicitudes de Viáticos' : 'Monitor y Control General de Viáticos';
  ...
  return (
    <div className="flex h-screen min-h-0 min-w-0 w-full bg-[#f8fafc] font-sans text-[#1e293b] overflow-hidden">
      ...
      {/* Main Content Area: min-w-0 is critical on mobile so wide tables/forms cannot expand the flex item. */}
      <div
        className={`flex-1 min-w-0 min-h-0 flex flex-col h-full overflow-hidden ${
          currentUser ? 'bg-slate-100/90' : 'bg-gradient-to-br from-slate-950 via-[#0b1329] to-slate-950'
        }`}
      >
        <header
          className={`h-14 min-w-0 border-b flex items-center justify-between px-4 sm:px-6 shrink-0 z-10 ${
            currentUser ? 'bg-white border-slate-200' : 'bg-slate-950/80 backdrop-blur-md border-slate-800 text-white'
          }`}
        >
          <div className="flex items-center gap-3 min-w-0">
            <button
              ...
              className={`hidden lg:flex p-1.5 rounded-lg transition cursor-pointer ${currentUser ? 'text-slate-500 hover:text-slate-900 hover:bg-slate-100' : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}
            ...
            <button
              ...
              className={`lg:hidden p-1.5 rounded-lg shrink-0 ${currentUser ? 'text-slate-600 hover:bg-slate-100' : 'text-slate-300 hover:bg-slate-800'}`}
            >
              <Menu className="w-5 h-5" />
            </button>
            <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse shrink-0" />
            <h2 className={`text-xs sm:text-sm font-bold truncate min-w-0 ${currentUser ? 'text-slate-800' : 'text-white'}`}>
              {getTabTitle()}
            </h2>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <div className={`hidden md:flex items-center gap-1.5 border px-2 py-1 rounded-lg text-[10px] ${currentUser ? 'bg-slate-100 border-slate-200 text-slate-600' : 'bg-slate-900 border-slate-800 text-slate-400'}`}>
              ...
              <button
                type="button"
                onClick={() => currentUser ? handleLogout() : handleSwitchUser('sistemas@dimer.com.mx')}
                className={`px-2 py-0.5 rounded font-bold transition flex items-center gap-1 cursor-pointer ${currentUser ? 'bg-indigo-600 text-white shadow-xs' : 'bg-slate-800 hover:bg-slate-700 text-slate-200'}`}
              >
            ...
            {currentUser ? (
              <div className="flex items-center gap-2">
                <div className="hidden sm:flex items-center gap-1.5 bg-slate-100 border border-slate-200 px-2.5 py-1 rounded-md text-[10px] text-slate-600">
                  ...
                  <span className={`font-bold px-1.5 py-0.5 rounded text-[9px] uppercase font-mono ${userRole === 'ADMIN' ? 'bg-purple-100 text-purple-800' : userRole === 'SOLO_LECTURA_APROBADAS' ? 'bg-emerald-100 text-emerald-800' : 'bg-blue-100 text-blue-800'}`}>
              ...
              <div className="flex items-center gap-1.5">
                <button type="button" onClick={() => { setAuthModalMode('login'); setAuthModalOpen(true); }} className="px-2.5 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 rounded-md text-[11px] font-bold transition flex items-center gap-1 cursor-pointer">
            ...
            {currentUser && activeTab !== 'solicitar' && !isSoloLectura && (
              <button type="button" onClick={() => setActiveTab('solicitar')} className="bg-[#0f172a] hover:bg-indigo-600 text-white px-3 py-1.5 rounded-md text-[11px] font-bold tracking-wide transition-colors flex items-center gap-1.5 shadow-sm cursor-pointer">
