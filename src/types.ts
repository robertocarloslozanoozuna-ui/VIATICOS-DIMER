...
const LEGACY_ROLE_IDS:Record<string,string>={ADMIN:'role_admin',JEFE:'role_jefe',FINANZAS:'role_finanzas',SOLO_LECTURA_APROBADAS:'role_solo_lectura',SOLICITANTE:'role_solicitante',EMPLEADO:'role_empleado'};
...
export function userHasRole(user:User|null|undefined,role:Role):boolean{if(!user)return false;const wantedName=String(role||'').trim().toUpperCase();const wantedId=LEGACY_ROLE_IDS[wantedName]||String(role||'').trim();if(String(user.role||'').toUpperCase()===wantedName)return true;if(getUserRoleIds(user).includes(wantedId))return true;return Boolean(user.roles?.some(r=>String(r.name||'').trim().toUpperCase()===wantedName));}
